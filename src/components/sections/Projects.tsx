        ...
        <div className="mt-16 space-y-8">
          {resumeData.projects.map((project, i) => {
            ...
            return (
              <div
                key={i}
                className="shimmer-card group relative p-8 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-2"
                style={{
              ...
              >
                ...
                <div className="flex items-start gap-4 mb-5">
                  ...
                  >
                    {i === 0 ? <Code2 size={22} style={{ color: a.icon }} /> : <Terminal size={22} style={{ color: a.icon }} />}
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-white mb-1">{project.name}</h3>
                    <div className="flex flex-wrap gap-4">
                      <div className="flex items-center gap-1.5 text-slate-400 text-xs">
                        <Building size={12} /><span>{project.org}</span>
                      </div>
                      {project.period && (
                        <div className="flex items-center gap-1.5 text-slate-400 text-xs">
